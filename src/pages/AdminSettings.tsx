  ...
  return (
    <div className="space-y-8 max-w-4xl">
      <header>
        ...
        <p className="text-slate-500 mt-1">Manage IP addresses and domains being monitored.</p>
      </header>
      ...
      <section className="bg-slate-900 rounded-2xl border border-slate-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-800">
          <h2 className="text-lg font-medium text-slate-200">Monitored Targets ({targets.length})</h2>
        </div>
                ...
                <tr key={t.id} className="hover:bg-slate-800/50 transition-colors group">
                  <td className="py-4 px-6">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium ${
                      t.type === 'domain' ? 'bg-indigo-500/10 text-indigo-400' : 'bg-purple-500/10 text-purple-400'
                    }`}>
